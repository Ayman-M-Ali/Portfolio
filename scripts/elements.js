export const form = document.querySelector('form');
export const theName = document.querySelector('.contact__input[type="text"]');
export const email = document.querySelector('.contact__input[type="email"]');
export const phone = document.querySelector('.contact__input[type="tel"]');
export const textArea = document.querySelector('.contact__textarea');
export const errorDiv = document.querySelector('.show-error');
export const sectionElements = document.querySelectorAll('.section');